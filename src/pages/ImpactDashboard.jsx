...
import OilPalmExpansionSimulator from "./OilPalmExpansionSimulator";
...
export default function ImpactDashboard() {
  ...
  // Add these state variables with other useState declarations in ImpactDashboard component
const [cpoTarget, setCpoTarget] = useState("150000"); // 11.2 lakh tonnes in tonnes
const [cpoScope, setCpoScope] = useState("all_india");
const [cpoBaseline, setCpoBaseline] = useState("latest");
const [cpoStartMonth, setCpoStartMonth] = useState("4");
const [cpoResult, setCpoResult] = useState(null);
const [loadingCpo, setLoadingCpo] = useState(false);
const [cpoError, setCpoError] = useState(null);
const [cpoStates, setCpoStates] = useState([
  "Andhra Pradesh",
  "Telangana",
  "Karnataka",
  "Tamil Nadu",
...
const [selectedCpoState, setSelectedCpoState] = useState(cpoStates[0]); // Default to first state

// Update fetchCpoTargetTimeline function
const fetchCpoTargetTimeline = async () => {
  if (!cpoTarget || parseFloat(cpoTarget) <= 0) {
    setCpoError("Please enter a valid CPO production target");
    return;
  }

  if (cpoScope === "state" && !selectedCpoState) {
    setCpoError("Please select a state for state-wise analysis");
    return;
  }
  ...
  try {
    const requestData = {
      target: parseFloat(cpoTarget),
      scope: cpoScope,
      baseline: cpoBaseline,
      start_month: parseInt(cpoStartMonth)
    };

    // Add state parameter if scope is state
    if (cpoScope === "state") {
      requestData.state = selectedCpoState;
    }

    const response = await fetch("http://localhost:5000/cpo-target-timeline", {
      method: "POST",
      headers: { 
    ...
    console.log("CPO Target Timeline Response:", data);
    setCpoResult(data);
  } catch (err) {
    console.error("CPO Target Timeline Error:", err);
    setCpoError(err.message || "Failed to fetch CPO target timeline. Please check connection.");
  } finally {
    setLoadingCpo(false);
  }
};

// Add this helper function to get month name from number
...
<OilPalmExpansionSimulator selectedState={stateFilter} />
...
<div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-6 overflow-hidden">
  <div className="bg-gradient-to-r from-[#0072bc] to-[#00509e] text-white p-4">
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="text-lg font-bold">CPO Production Target Timeline</h3>
          <p className="text-sm opacity-90">
            Predict when CPO production targets will be reached based on monthly production patterns
          </p>
        </div>
      </div>
      <div className="text-xs px-2 py-1 bg-white/20 rounded">
  ...
  <div className="p-6">
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2">
        <div className="bg-gray-50 rounded-lg border border-gray-200 p-5">
          <h4 className="font-bold text-gray-800 mb-4">Production Timeline Parameters</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-300 p-3 focus:border-blue-500 focus:ring-blue-500"
                  value={cpoTarget}
                  onChange={(e) => setCpoTarget(e.target.value)}
                  placeholder="Enter target (e.g., 1120000)"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  value={cpoScope}
                  onChange={(e) => {
                    setCpoScope(e.target.value);
                    // If switching to state-wise and no state selected, use first available
                    if (e.target.value === "state" && !selectedCpoState && cpoStates.length > 0) {
                      setSelectedCpoState(cpoStates[0]);
                    }
                  }}
                >
                ...
                <div className="text-xs text-gray-500 mt-1">
                  {cpoScope === "state" 
                    ? `Analyze production for specific state` 
                    : "Analyze national production"}
                </div>
              </div>
            </div>
            
            {/* State selection - only shown when scope is state */}
            {cpoScope === "state" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                  ...
                  <select
                    className="w-full rounded-lg border border-gray-300 p-3 focus:border-blue-500 focus:ring-blue-500"
                    value={selectedCpoState}
                    onChange={(e) => setSelectedCpoState(e.target.value)}
                    disabled={cpoStates.length === 0}
                  >
                    {cpoStates.length === 0 ? (
                ...
                {/* <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Or Use Dashboard State
                  </label>
                  <button
                    onClick={() => {
                      if (stateFilter !== "All-India" && cpoStates.includes(stateFilter)) {
                        setSelectedCpoState(stateFilter);
                      } else if (cpoStates.length > 0) {
                        // If current dashboard state not in CPO states, use first available
                        setSelectedCpoState(cpoStates[0]);
                      }
                    }}
                    className="w-full px-4 py-3 bg-blue-50 text-blue-700 rounded-lg border border-blue-200 font-medium hover:bg-blue-100 transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                    disabled={cpoStates.length === 0}
                  >
                    <div className="flex items-center justify-center gap-2">
                      ...
                      {stateFilter !== "All-India" && cpoStates.includes(stateFilter) 
                        ? `Use ${stateFilter}`
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 gap-4 items-end">
              <div>
                ...
                <select
                  className="w-full rounded-lg border border-gray-300 p-3 focus:border-blue-500 focus:ring-blue-500"
                  value={cpoBaseline}
                  onChange={(e) => setCpoBaseline(e.target.value)}
                >
                  <option value="latest">Latest Year</option>
                  <option value="avg3">3-Year Average</option>
                </select>
                <div className="text-xs text-gray-500 mt-1">
              ...
              <div>
                ...
                <select
                  className="w-full rounded-lg border border-gray-300 p-3 focus:border-blue-500 focus:ring-blue-500"
                  value={cpoStartMonth}
                  onChange={(e) => setCpoStartMonth(e.target.value)}
                >
                  ...
                  <option value="11">November (Oil Year)</option>
                  <option value="12">December</option>
                </select>
                <div className="text-xs text-gray-500 mt-1">
                  Starting month for timeline
                </div>
              </div>
              
              <div className="flex items-end">
                <button
                  onClick={fetchCpoTargetTimeline}
                  disabled={loadingCpo || !cpoTarget || parseFloat(cpoTarget) <= 0 || (cpoScope === "state" && !selectedCpoState)}
                  className="w-full px-6 py-3 bg-[#003366] text-white rounded-lg font-medium hover:bg-[#164523] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {loadingCpo ? (
                    <>
                      <svg className="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          <div className="mt-6 pt-4 border-t border-gray-200">
            <h5 className="font-medium text-gray-700 mb-2">How it works:</h5>
            <ul className="text-sm text-gray-600 space-y-1 ml-5 list-disc">
              <li>Uses state-wise monthly production patterns from historical data</li>
              <li>Considers seasonal harvesting months for accurate predictions</li>
              <li>Available states: Andhra Pradesh, Karnataka, Tamil Nadu, Goa, Kerala, Mizoram, Telangana</li>
              <li>Calculates cumulative production month-by-month</li>
              <li>Accounts for multi-year targets when annual production is insufficient</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      {/* Results Panel */}
      <div>
        <div className="bg-gradient-to-br from-purple-50 to-blue-100 rounded-lg border border-purple-200 p-5 h-full">
          <h4 className="font-bold text-purple-900 mb-4">Timeline Results</h4>
          
          {cpoResult ? (
            <div className="space-y-5">
              <div className="text-center">
                <div className="text-xs text-gray-600 mb-1">Target Will Be Reached In</div>
                <div className="text-3xl font-bold text-purple-700">
                  {cpoResult.months_needed} months
                </div>
                <div className="text-sm text-gray-600 mt-1">
                  {(() => {
                    const m = Number(cpoResult.months_needed);
                    if (!m || Number.isNaN(m)) return "Month calculation not available";
                    const now = new Date();
                    const reached = new Date(now.getFullYear(), now.getMonth() + m, 1);
                    const monthName = getMonthName(reached.getMonth() + 1);
                    return `By ${monthName} ${reached.getFullYear()}`;
                  })()}
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-white rounded-lg p-4 border border-gray-200">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm text-gray-600">
                      {cpoResult.scope === "state" ? `${cpoResult.state} Target` : "National Target"}
                    </span>
                    <div className="text-right">
                      <div className="text-md font-bold text-purple-700">
                        {cpoResult.target?.toLocaleString() || "0"} tonnes
                      </div>
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <span className="text-sm text-gray-600">Cumulative at reach</span>
                    <div className="text-right">
                      <div className="text-md font-bold text-purple-600">
                        {cpoResult.production_by_the_time_target_will_reach?.toLocaleString() || "0"} tonnes
                      </div>
                ...
                <div className="p-3 rounded-lg border border-blue-200 bg-blue-50 text-blue-800">
                  <div className="font-medium mb-1">Annual Production Capacity</div>
                  <div className="text-sm">
                    {cpoResult.estimated_annual_total?.toLocaleString() || "0"} tonnes/year
                  </div>
                </div>
                
                <div className="text-sm text-gray-700 bg-white p-3 rounded border border-gray-200">
                  <div className="font-medium mb-2">Timeline Insights:</div>
                  <div className="space-y-1">
                    {cpoResult.months_needed <= 12 ? (
                      <>
                        <div className="flex items-center gap-2 text-green-600">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                          </svg>
                          <span>Target achievable within 1 year</span>
                        </div>
                        <div>Current production capacity is sufficient</div>
                      </>
                    ) : cpoResult.months_needed <= 24 ? (
                      <>
                        <div className="flex items-center gap-2 text-blue-600">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                          <span>Target requires 1-2 years</span>
                        </div>
                        <div>Moderate expansion needed to meet timeline</div>
                      </>
                    ) : cpoResult.months_needed <= 36 ? (
                      <>
                        <div className="flex items-center gap-2 text-amber-600">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                          <span>Target requires 2-3 years</span>
                        </div>
                        <div>Significant expansion efforts needed</div>
                      </>
                    ) : (
                      <>
                        <div className="flex items-center gap-2 text-red-600">
                          ...
                          <span>Long-term target (3+ years)</span>
                        </div>
                        <div>Major investment and expansion required</div>
                      </>
                    )}
                  </div>
                </div>
            ...
            <div className="text-center py-8">
              ...
              <h4 className="font-medium text-gray-700 mb-2">No Timeline Calculated</h4>
              <p className="text-sm text-gray-600">
                {cpoScope === "state" && !selectedCpoState 
                  ? "Select a state and enter target to calculate timeline"
                  : "Enter CPO production target and parameters, then click 'Calculate Timeline'"}
              </p>
            </div>
          )}
        </div>
