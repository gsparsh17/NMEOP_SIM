...
export default function App() {
  ...
  const NavItem = ({ id, label }) => (
    <button
      ...
      className={`w-full text-left px-4 py-2 text-sm rounded-lg transition ${
        activeScreen === id
          ? "bg-[#2F7F3E] text-white font-medium"
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {/* HEADER */}
      <header className="h-14 bg-[#234E2A] text-white flex items-center px-4 md:px-6 justify-between shadow">
        <h1 className="text-sm font-semibold">
          Ministry of Agriculture & Farmers Welfare — Policy Simulator
        </h1>

        {/* Hamburger icon for mobile */}
        <button
          className="md:hidden p-2 rounded hover:bg-[#2F7F3E]/30"
      ...
      <div className="flex flex-1">
        ...
        {/* SIDEBAR */}
        <aside
          className={`fixed md:static z-40 top-14 left-0 h-full w-56 bg-white shadow border-r border-slate-200 p-3 transform transition-transform duration-300 ${
            sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
          }`}
        >
          <p className="px-3 py-2 text-[11px] uppercase text-slate-500">
            Navigation
          </p>
          <NavItem id="overview" label="Overview Dashboard" />
          <NavItem id="scenario" label="Scenario Builder" />
          <NavItem id="impact" label="NMEO-OP Impact" />
          <NavItem id="diagnostics" label="Data & Model Diagnostics" />
        </aside>
